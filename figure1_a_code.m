...
for r = 1 : run_count  
    ...
    for j = 1 : size(theta,2)
        ...
        tmp_sub(j)  = mean( max(theta(:,j)'* (x - [opt_model.x]), 0) );
        tmp_pre(j)  = mean( sqrt(sum((x - [opt_model.x]).^2,1)) );
    end
...
yyaxis right
h2 = semilogx(epsilon,mean(Predictability,2),'linewidth',4,'color',[0.85, 0.325, 0.098], ...
         'linestyle','-.');
