...
param(1:run_count) = struct('W',[eye(n);-eye(n)],'H',[zeros(n,m);zeros(n,m)], ...
                            ...
                            'd',0,'alpha',0.9,'solver','mosek','tol',0, ...
                            'epsilon2',epsilon2,'delta',0.2,'pnorm',1, ...
                            'kernel','gauss', 'sigma', sigma, 'kappa', kappa, ...
...
Suboptimality3  = zeros(4,run_count);
Predictability3 = zeros(4,run_count);
...
for r = 1 : run_count  
    ...
    % Check SAA Solution
    opt_SAA = Quadratic_Model(param(r),Q_xx(:,:,1),Q_xs(:,:,1),q(:,:,1),s);
    tmp1(2) = mean(diag(x_star'*Q_xx(:,:,1)*x_star)' + diag(x_star'*Q_xs(:,:,1)*s)' + q(:,:,1)'*x_star - [opt_SAA.objective]);
    tmp2(2) = mean( sqrt(sum((x_star - [opt_SAA.x]).^2,1)) );
    % Check DRO Solution
...
for r = 1 : run_count  
    ...
    tmp_pre = zeros(length(sigma),length(kappa),kfold);
    pw = fix(N/kfold);
    for k = 1 : kfold
        s_tr = s(:,1:N);
        x_tr = x_star(:,1:N);
        ...
        for i = 1 : length(sigma)
            for j = 1 : length(kappa)                
                param(r).sigma = sigma(i);
                opt_gupta_n = Steepest_Descent_For_Gupta(param(r),opt_inv_ng(i,j).alpha,x_tr,s_v);
    ...
    param(r).sigma = sigma(i(end)); 
    param(r).kappa = kappa(j(end));
    ...
    opt_gupta_n = Steepest_Descent_For_Gupta_Matlab(param(r),alpha_ng,x_tr,s);
    tmp2(2) = mean( sqrt(sum((x_star - opt_gupta_n.x).^2,1)) );
    % Check SAA Solution
    opt_SAA = Quadratic_Model(param(r),Q_xx(:,:,1),Q_xs(:,:,1),q(:,:,1),s);
    tmp1(3) = mean(diag(x_star'*Q_xx(:,:,1)*x_star)' + diag(x_star'*Q_xs(:,:,1)*s)' + q(:,:,1)'*x_star - [opt_SAA.objective]);
    tmp2(3) = mean( sqrt(sum((x_star - [opt_SAA.x]).^2,1)) );
    % Check DRO Solution
    opt_DRO = Quadratic_Model(param(r),Q_xx(:,:,2),Q_xs(:,:,2),q(:,:,2),s);
    tmp1(4) = mean(diag(x_star'*Q_xx(:,:,2)*x_star)' + diag(x_star'*Q_xs(:,:,2)*s)' + q(:,:,2)'*x_star - [opt_DRO.objective]);
    tmp2(4) = mean( sqrt(sum((x_star - [opt_DRO.x]).^2,1)) ); 
    
    Suboptimality3(:,r) = tmp1;
    Predictability3(:,r) = tmp2;
end
%% Table in Latex
fileID = fopen('.\tables\table3.tex','w');
...
fprintf(fileID,'\\multirow{3}{*}{Noisy training } &  Gupta (Parametric)');
...
fprintf(fileID,'\\multirow{3}{*}{Suboptimal training } & Gupta (Parametric)');
...
fprintf(fileID,'\\multirow{4}{*}{Model mismatch } & Gupta (Parametric)');
str1 = value2latex(mean(Suboptimality3(1,:)));
str2 = value2latex(mean(Predictability3(1,:)));
fprintf(fileID,'& $%s$ & $%s$ \\\\ \n',str1,str2);
fprintf(fileID,' & Gupta (Semi-Parametric) ');
str1 = '-';
