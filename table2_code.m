...
for r = 1 : run_count  
    ...
    for n = all_n
        for m = all_m
            ...
            tmp1(1,ind_n,ind_m) = mean( max(theta_gupta'* (x - [opt_gupta.x]) - delta, 0) );
            ...
            tmp1(2,ind_n,ind_m) = mean( max(theta(:,1)'* (x - [opt_SAA.x]) - delta, 0) );
            ...
            tmp1(3,ind_n,ind_m) = mean( max(theta(:,2)'* (x - [opt_DRO.x]) - delta, 0) );
